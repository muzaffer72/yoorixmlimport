...
export class ObjectStorageService {
  ...
  // Generate multiple image sizes from uploaded image
  async generateImageSizes(originalUrl: string): Promise<{
    image_40x40: string;
    image_72x72: string;
    image_190x230: string;
    image_210x205?: string;
    image_405x745?: string;
    storage: string;
    original_image: string;
  }> {
    // For now, we'll create the expected format with the same image
